...
# -----------------------------
data_file = r"C:\Users\esisy\Downloads\rollingsales_brooklyn.xlsx"  # Changed to raw string

# Read the Excel file (make sure this file exists at the given location)
try:
    df = pd.read_excel(data_file)
except FileNotFoundError:
    print(f"Error: File not found at {data_file}.  Please check the file path.")
    exit()
...
if 'SALE PRICE' in df.columns:
    ...
    print("Warning: 'SALE PRICE' column not found in the DataFrame.")
    exit()

# Remove rows with missing or zero sale price
df = df[df['SALE PRICE'] > 0].copy()
df.dropna(subset=['SALE PRICE'], inplace=True) #handles NaN values in 'SALE PRICE'
...
if not ('x' in df.columns and 'y' in df.columns):
    ...
    print("\nSynthesized 'x' and 'y' coordinates.")
else:
    print("\n'x' and 'y' coordinates found in the data.")

# ----------------------------------------------------------
# STEP 3: Build a scalar field from sale price via interpolation
# ----------------------------------------------------------
# Define a grid over the geographic region
num_points = 100 # Increased the number of points for finer resolution
xi = np.linspace(df['x'].min(), df['x'].max(), num_points)
yi = np.linspace(df['y'].min(), df['y'].max(), num_points)
...
if np.any(mask):
    ...
    print("\nNaN values in interpolated grid filled with nearest neighbor interpolation.")
...
center_i, center_j = num_points // 2, num_points // 2 # changed to use num_points
...
print(f"Curl (z-component) at center: {curl_z[center_i, center_j]:.2e}")
...
# Increased the density of quiver plot
plt.quiver(X_grid[::5, ::5], Y_grid[::5, ::5], grad_x[::5, ::5], grad_y[::5, ::5], color='white', scale=50) # Added scale
plt.xlabel('x')
plt.ylabel('y')
...
fig = plt.figure(figsize=(10, 8))
ax = fig.add_subplot(111, projection='3d')
surf = ax.plot_surface(T, R_vals, P, cmap='plasma')
...
fig = plt.figure(figsize=(10, 8))
ax = fig.add_subplot(111, projection='3d')
ax.bar(bin_centers, hist, zs=0, zdir='y', alpha=0.7, width=(bin_edges[1]-bin_edges[0])*0.8) #Added width
